...
exports.getProdutosCompleto = async (req, res) => {
  let sql =
    ...
    "c.id_categoria, p.codigo, p.imagem, p.ativo, count(pt.id_tamanho) qtd_tamanhos, " +
    ...
    "GROUP BY p.id_produto, c.descricao_categoria, pt.pontos, t.descricao, pt.id_produtos_tamanhos, t.id_tamanho, c.id_categoria, p.ativo " +
...
exports.salvarProduto = async (req, res) => {
  ...
  try {
    const resultProduto = await pg.execute(
      "INSERT INTO produtos(descricao, codigo, imagem, ativo, id_categoria) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [descricao_produto, codigo_produto, link_imagem, ativo, id_categoria]
    );
