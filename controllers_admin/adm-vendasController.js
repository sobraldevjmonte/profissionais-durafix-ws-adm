...
exports.atualizarStatusPedido = async (req, res) => {
  const { id_venda, status } = req.body;
...
exports.listaTodosPedidos = async (req, res) => {
  ...
  try {
    const result = await pg.execute(
      "SELECT v.id_vendas, v.data_lancamento, v.data_venda, v.numero_venda, v.total_pontos, v.status, c.nome_cliente, u.nome, v.rejeicoes, v.motivo_rejeicao, v.premiado " +
        "FROM vendas v, clientes c, usuarios u " +
        "WHERE v.id_cliente = c.id_cliente " +
        "AND v.id_usuario = u.id_usuario " +
